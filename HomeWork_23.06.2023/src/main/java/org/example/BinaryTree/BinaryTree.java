...
/*        tree.insert(60);
        ...
        while (exit){
            ...
            System.out.println("F -> Если хочешь узнать семметрично ли дерево. ");
            ...
            switch (choice) {
                ...
                case "E" -> findNumber(tree);

                case "F" -> {
                    boolean symmetric = tree.isSymmetric(tree.root);
                    if(symmetric){
                        System.out.println("Дерево является симметричным.");
                    } else {
                        System.out.println("Дерево не является симметричным.");
                    }
                }
